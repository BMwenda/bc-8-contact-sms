...
class Contact(Base):
    ...
    def __repr__(self):
        return "<Contact('%s', '%s', '%s', '%s')>" % (self.first_name, self.last_name, self.phone_number, self.is_sync, self.user_id)
...
class Message(Base):
    ...
    def __repr__(self):
        return "<Message('%s', '%s', '%s', '%s')>" % (self.source, self.recipient, self.message_body, self.date_sent, self.user_id)
